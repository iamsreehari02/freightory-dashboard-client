  ...
  return (
    <Card className="max-w-none">
      ...
      <CardContent className="space-y-4">
        {isLoadingLogs && <p className="text-sm text-muted-foreground">Loading...</p>}

        {!isLoadingLogs && logs.length === 0 && (
          <p className="text-sm text-muted-foreground">No branch activity found.</p>
        )}
            ...
            >
              ...
              <div className="flex-1 flex flex-col gap-1">
                <span className="font-medium">
                  {log.action.charAt(0).toUpperCase() + log.action.slice(1)} Branch
                </span>
                <span className="text-sm text-muted-foreground capitalize">{log.message}</span>
