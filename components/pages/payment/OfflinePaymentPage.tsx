...
import { BankDetails, getBankDetails } from "@/services/api/bank";
...
export default function OfflinePaymentPageComponent() {
  ...
  useEffect(() => {
    const fetchPaymentSummary = async () => {
      try {
        ...
        const companyCurrency = summary?.currency || "USD";

        if (companyCurrency !== "INR") {
          const rate = await getConversionRate(companyCurrency);
        ...
        setUserCurrency(companyCurrency);
      } catch (err) {
        console.error(err);
            ...
            <Form {...form}>
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Upload Receipt (Image Only)</FormLabel>
                      <FormControl>
                        <Input
                          type="file"
                          accept="image/*"
                      ...
                      <FormDescription>
                        Only image files allowed. Max file size: 5MB
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
